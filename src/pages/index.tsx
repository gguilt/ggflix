  ...
  return (
    <>
      ...
      <main className={styles.main}>
        <h1 className={styles.logo}>GGFLIX</h1>
        <h2 className={styles.description}>TV Show Search App</h2>

        <form className={styles.form} onSubmit={searchSubmit}>
          <input placeholder="Enter TV Show Name" value={searchTerm} onChange={setInput}></input>
          ...
          {showData.map((e: any) => {
            return (
                <div className={styles.listItem} key={e.show.id}>
                  ...
                  {e.show.image && <img alt={e.show.name} src={e.show.image.medium} />}
                </div>
              );
          })}
        </div>
